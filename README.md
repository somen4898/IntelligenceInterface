...
LLM coding agents burn thousands of tokens reading whole files and wading through noisy grep results to answer simple structural questions. ii-structure replaces "read text and filter mentally" with "ask a structural question, get a structural answer." All 13 commands (8 read, 2 write, 3 analysis) work identically across Python, Go, and TypeScript.
...
- **Fast.** Fresh build in <0.5s (parallel parsing). Structural commands complete in <120ms. Type-resolved `usages` in <1s.
...
- **Incremental.** Only re-parses changed files on refresh. No-change refreshes skip expensive post-processing entirely (~20ms).
...
| `overview` | Graph-powered project overview — structure, key files, entry points | `ii-structure overview` |
...
```
src/ii_structure/
├── cli.py              # Click entry point (command dispatch)
├── parser.py           # Python ast-based symbol/import/edge extraction
├── index.py            # Structural index with parallel parsing and incremental refresh
├── graph.py            # SQLite graph store (nodes, edges, FTS5 search, analysis queries)
├── migrations.py       # Versioned schema migration framework
...
└── commands/
    ...
    ├── overview.py     # Graph-powered project overview
    ...
    ├── init_cmd.py     # CLAUDE.md generation
    └── benchmark_cmd.py# Benchmark runner
```
...
| Operation | Time |
|-----------|------|
| Fresh build (82 files) | 0.45s |
| No-change refresh | 0.02s |
| search / locate / outline | ~115ms |
| blast-radius (156 nodes) | ~130ms |
| FTS5 index rebuild | included in build |

Key optimizations:
- **Parallel parsing** — `ProcessPoolExecutor` for file parsing (3.5x faster builds)
- **git ls-files** — file discovery respects `.gitignore` natively, faster than `rglob("*")`
- **FTS5 search** — porter-stemmed full-text search with LIKE fallback
- **SQLite tuning** — WAL mode, `synchronous=NORMAL`, 8MB cache, memory-mapped I/O
- **Indexed lookups** — `WHERE name = ?` on indexed column instead of Python loops
- **Batch queries** — `batch_get_nodes()` replaces N+1 `get_node()` calls
- **Recursive CTEs** — blast-radius and test-coverage use SQL CTEs instead of Python BFS
- **Schema migrations** — versioned, idempotent migrations (v1-v6) so schema evolves without full rebuilds
